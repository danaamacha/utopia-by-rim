...
export default function OrderConfirmation() {
  ...
  const promoDiscount = promo ? (promo.discountAmount ?? (subtotal * (promo.percent || 0)) / 100) : 0;
  const freeShipping = promo?.freeShipping ?? false;
  const actualShippingCost = freeShipping ? 0 : shippingCost;
  const total = Math.max(0, subtotal - promoDiscount) + actualShippingCost;
        ...
        <div style={shell}>
          <div style={card}>
            ...
            >
              ...
              <Row label={`Shipping (${shipping === "express" ? "Express" : "Standard"})`} value={freeShipping ? "Free" : fmt(actualShippingCost)} />
